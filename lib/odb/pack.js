...
const GIT_OBJ_OFS_DELTA = 6;
const GIT_OBJ_REF_DELTA = 7;
...
var PackFile = function(parent, dir, name) {
  this.parent = parent;
  this.dir = dir;
  this.name = name;
  this.idx = null;
          ...
          if(tmpBuf[0] & 0x80) {
            ...
            fs.read(fd, targetBuffer, 0, targetBuffer.length, ++initialOffset, function(err, nBytes) {
              if(err) exit.from('readTarget')(err); else if(nBytes < targetBuffer.length) exit(new Error('Corrupted Pack')); else try {
                if(PackFile.zlibTypes[type-1]) {
                  inflate(targetBuffer, function(err, data) {
                ...
                } else if(type === GIT_OBJ_OFS_DELTA) {
                  // oh god it's an OFS delta.
                  ...
                  self.readObjectAt({offset:newOffset}, offset, function(err, data) {
                    ...
                    inflate(deltaBuffer, function(err, data) {
                  ...
                  var targetOID = oid.OID.fromRaw(targetBuffer.slice(0, 20));

                  inflate(targetBuffer.slice(20), function(err, data) {
                      self.pack.parent.parent.repo.lookup(targetOID, function(err, raw) {
                        if(err) exit.from('refdelta lookup')(err); else {
                          var dataAsBuffer = new Buffer(Buffer.byteLength(data, 'binary')),
                              rawData = raw.asBuffer(),
                              rawAsBuffer = new Buffer(Buffer.byteLength(rawData, 'binary')),
                              outgoing;

                          rawAsBuffer.write(rawData, 'binary');
                          dataAsBuffer.write(data, 'binary');
                          for(var i = 0, len = rawAsBuffer.length; i < len; ++i) {
                            if(rawAsBuffer[i] === 0) {
                              break;
                            }
                          }
                          outgoing = ofs.applyDelta(dataAsBuffer, rawAsBuffer.slice(i+1));
                          for(var i = 0, len = outgoing.length; i < len; ++i) {
                            if(outgoing[i] === 0) {
                              break;
                            }
                          }
                          exit(null, {type:GitObject.types_to_packed[raw.type], data:outgoing.slice(i).toString('binary')});
                        }
                      });
                  });
                } else {
    ...
    fs.readdir(packDir, function(err, files) {
      if(err) {
        ...
        files.filter(function(file) {
          ...
          var pack = new PackFile(self, packDir, packFile.replace(/\.pack$/, ''));
