...
var oidParse = function(info) {
  return tempis.oid.fromHex(info); 
};
...
var personParse = function(info) {
  ...
  return {
    'name':bits[0],
    'email':bits[2],
    'datetime':Date.parse((new Date(parseInt(bits[4], 10))) + bits[4].split(' ').slice(-1)[0])
  }; 
};
...
GitCommit.parse = function(repo, oid, buf) {
  ...
  var beginMessageIDX = str.indexOf('\n\n'),
      head = buf.slice(0, beginMessageIDX),
      tail = buf.slice(beginMessageIDX+2);
